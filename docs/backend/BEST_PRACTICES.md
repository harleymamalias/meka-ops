...
Frontend component naming is defined separately in [FRONTEND_ARCHITECTURE.md](../frontend/FRONTEND_ARCHITECTURE.md): MekaOps-owned React components use PascalCase files and folders, while shadcn-managed primitives retain the generated lowercase convention.
...
describe('VehiclesService', () => {
  ...
  it('should throw NotFoundException if vehicle not found', async () => {
    ...
    await expect(service.findByIdOrFail('bad-id')).rejects.toThrow(
      NotFoundException,
    );
  });
});
